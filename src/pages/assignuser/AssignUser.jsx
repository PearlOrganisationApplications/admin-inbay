...
const AssignUser = () => {
  ...
  const fetchAssignedUsers = async (managerId) => {
    if (!managerId) {
      setAssignedUsers([]);
      return;
    }
    ...
    try {
      ...
      if (Array.isArray(result)) {
        ...
        assignedData = result.users;
      } else if (Array.isArray(result?.data?.users)) {
        assignedData = result.data.users;
      }
  ...
  const handleManagerSelect = async (manager) => {
    ...
    await fetchAssignedUsers(manager.id);
  };
  ...
  const handleUserSelect = (user) => {
    if (isAlreadyAssigned(user.id)) {
      showModal(
        "warning",
        "Already Assigned",
        `${user.name} is already assigned to ${selectedManager?.name}.`
      );
      return;
    }
  ...
  const handleSelectAll = () => {
    ...
    const selectableUsers = filteredUsers.filter(
      (user) => !isAlreadyAssigned(user.id)
    );
    ...
    if (selectableUsers.length === 0) {
      showModal(
        "warning",
        "No Users Available",
        "All users in the current list are already assigned to this manager."
      );
      return;
    }
    ...
    if (allSelected) {
      ...
      setSelectedUsers((prev) => {
        const existingIds = new Set(
          prev.map((user) => user.id)
        );

        const newUsers = selectableUsers.filter(
  ...
  const handleAssign = async () => {
    ...
    try {
      ...
      await fetchAssignedUsers(selectedManager.id);
      await fetchData();
    } catch (error) {
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {isSelected && (
                        <p className="text-[10px] text-[#8b2cf5] font-bold mt-1">
                          Manager ID: {mgr.id}
                        </p>
                      )}
                    </div>
            ...
            <div className="bg-white rounded-3xl border border-purple-100 shadow-sm overflow-hidden">
              <div className="px-5 py-4 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-xl bg-purple-100 text-[#8b2cf5] flex items-center justify-center">
                    <UserCheck size={20} />
                  </div>
                ...
                <button
                  ...
                  className="flex items-center justify-center gap-2 px-4 py-2 rounded-xl bg-purple-50 text-[#8b2cf5] text-xs font-bold hover:bg-purple-100 transition-all disabled:opacity-50"
                >
                  <RefreshCw
                    size={14}
                    className={
              ...
              {assignedLoading ? (
                <div className="py-10 flex flex-col items-center justify-center">
                  <Loader2 className="w-8 h-8 text-[#8b2cf5] animate-spin" />
                ...
                <div className="py-10 flex flex-col items-center justify-center text-center">
                  ...
                  <p className="text-xs text-gray-400 mt-1">
                    This manager currently has no users assigned.
                  </p>
                </div>
              ) : (
                <div className="p-4">
                      ...
                      >
                        <div className="flex-shrink-0">
                          {user.profile_image ? (
                            ...
                            <div className="w-11 h-11 rounded-full bg-purple-100 text-[#8b2cf5] flex items-center justify-center font-black">
                              {user.name
                                ?.charAt(0)
                                ?.toUpperCase() || "U"}
                            </div>
            ...
            {!selectedManager ? (
              <div className="flex flex-col items-center justify-center py-20 text-center">
                ...
                <p className="text-sm text-gray-400 mt-1">
                  Choose a manager from the left to view assigned users and select new users.
                </p>
              </div>
                ...
                {filteredUsers.map((user) => {
                  ...
                  return (
                    <div
                      ...
                      className={`p-4 flex items-center gap-4 transition-all relative ${
                        alreadyAssigned
...
const ResultModal = ({ modal, onClose }) => {
  ...
  const {
    ...
    config[modal.type] || config.success;
